...
class SmartFactoryConfig:  # Define configuration class for smart factory simulation
    ...
    def __init__(self):  # Initialize the configuration object with all parameters
        ...
        self.num_time_steps = 10  # Set total time steps to 60,000 for 60 seconds of simulation at 1ms intervals
        ...
        self.bs_position = [10.0, 10.0, 4.5]  # Position BS at center of ceiling (x=10, y=10, z=4.5)
        ...
        self.materials = {  # Define material properties for ray tracing
            'concrete': {  # Properties for concrete material (walls, floor, ceiling)
                ...
                'relative_permittivity': 5.31,  # Set relative permittivity for concrete
                'conductivity': 1.0e7,  # Define high conductivity for concrete (S/m)
                'roughness': 0.1e-3,  # Set surface roughness to 0.1 mm
                'scattering_coefficient': 0.5,  # Define scattering coefficient for diffuse reflection
        ...
        self.agv_trajectories = {
            'agv_1': [[2.0, 5.0], [18.0, 5.0], [2.0, 5.0]],  # Behind Shelf 1 and 2
            'agv_2': [[17.0, 17.0], [17.0, 14.0], [3.0, 14.0], [3.0, 11.0], [17.0, 11.0], [17.0, 17.0]]
        }
        ...
        self.agv_movement = {  # Configure AGV movement behavior
            ...
            'update_interval': 0.5,  # Update AGV position every 0.5 seconds
            'obstacle_avoidance': True,  # Enable basic obstacle avoidance (though paths are fixed)
        ...
        self.agv_positions = [  # Set starting positions for AGVs explicitly
            [2.0, 5.0, 0.5],  # AGV 1 starts at bottom-left corner, 0.5m height
            [17.0, 17.0, 0.5]  # AGV 2 starts at top-right corner, 0.5m height
        ]
        ...
        self.static_scene = {  # Define static environment properties
            ...
            'reflectivity': 0.603815  # Define concrete reflectivity at 28 GHz (used as reference)
        }
        ...
        self.scene_objects = {  # Define obstacles (shelves) in the scene
            'num_shelves': 5,  # Set number of shelves to 5
            'shelf_dimensions': [  # Define dimensions for each shelf (width, depth, height)
                [4.0, 1.0, 4.0],  # Shelf 1: 4m wide, 1m deep, 4m tall
                [2.0, 1.0, 4.0],  # Shelf 2: 2m wide, 1m deep, 4m tall
                [1.0, 2.0, 5.0],  # Shelf 3: 1m wide, 2m deep, 5m tall
                [1.0, 2.0, 5.0],  # Shelf 4: 1m wide, 2m deep, 5m tall
                [4.0, 1.0, 5.0]   # Shelf 5: 4m wide, 1m deep, 5m tall
            ],
            'shelf_material': 'metal',  # Set shelf material to metal
            'shelf_positions': [  # Define positions for each shelf (x, y, z)
                [4.0, 4.0, 0.0],  # Shelf 1 at (4, 4, 0)
                [12.0, 4.0, 0.0],  # Shelf 2 at (12, 4, 0)
                [4.0, 10.0, 0.0],  # Shelf 3 at (4, 10, 0)
                [14.0, 12.0, 0.0],  # Shelf 4 at (14, 12, 0)
                [8.0, 16.0, 0.0]   # Shelf 5 at (8, 16, 0)
            ],
            'shelf_orientation': [0.0, 0.0, 0.0]  # Set shelf orientation (no rotation)
        }
        ...
        self.beamforming = {  # Configure beamforming settings
            'num_beams': 512,  # Define 128 beams for fine-grained directionality
            ...
            'codebook': {  # Configure beamforming codebook
                ...
                'size': 512,  # Set codebook size to 128 beams
