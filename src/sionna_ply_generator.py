...
class SionnaPLYGenerator:
    ...
    def generate_factory_geometries(config, output_dir):
        ...
        try:
            ...
            # Generate floor with itu_concrete material
            SionnaPLYGenerator._generate_horizontal_surface(
                ...
                z=config.room_dim[2],  # Ceiling height
                material_type='itu_concrete'  # Use consistent material name  # Explicitly specify concrete material
            )
            ...
            SionnaPLYGenerator._generate_horizontal_surface(
                ...
                material_type='itu_concrete'  # Use itu_concrete instead of 'ceiling'
            )
    ...
    @staticmethod
    def _generate_horizontal_surface(filename, width, depth, z, material_type='itu_concrete'):
        """
        Generate horizontal surface (floor or ceiling) PLY with material properties
        ...
        Args:
            ...
            material_type: Material name ('itu_concrete' or 'itu_metal')
        """
        try:
            # Material ID mapping
            material_ids = {
            ...
            with open(filename, 'wb') as f:
                ...
                f.write(b'element face 2\n')  # Changed to 2 faces (triangles) instead of 1 quad
                ...
                vertices = [
                    (0.0, 0.0, z, 0.0, 0.0),      # Bottom-left (0)
                    (width, 0.0, z, 1.0, 0.0),    # Bottom-right (1)
                    (width, depth, z, 1.0, 1.0),   # Top-right (2)
                    (0.0, depth, z, 0.0, 1.0)      # Top-left (3)
                ]
                ...
                # Write face data - two triangles instead of one quad
                # First triangle (0,1,2)
                f.write(struct.pack('<B', 3))  # 3 vertices for first triangle
                f.write(struct.pack('<3i', 0, 1, 2))  # vertex indices
                f.write(struct.pack('<i', material_id))  # material index

                # Second triangle (0,2,3)
                f.write(struct.pack('<B', 3))  # 3 vertices for second triangle
                f.write(struct.pack('<3i', 0, 2, 3))  # vertex indices
                f.write(struct.pack('<i', material_id))  # material index
